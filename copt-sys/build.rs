...
fn copt_home() -> String {
  let var = env::var("COPT_HOME").expect("failed to retrieve the value of COPT_HOME");
  if !Path::new(&var).exists() {
    panic!("COPT_HOME is invalid path");
  }
  var
}
...
//   append_path(PathBuf::from(copt_home()).join("bin"));

//   let output = Command::new("coptampl").arg("-v").output().expect("failed to execute coptampl");
...
fn main() {
  let copt_home = copt_home();
  let libpath: PathBuf = PathBuf::from(copt_home).join("lib");

  //let (major, minor, _) = get_version_triple();
  //let libname = format!("copt{}{}", major, minor);
  let libname = "copt";
